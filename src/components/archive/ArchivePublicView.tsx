...
// Keep long collections quick to open while still making every item available
...
export const ArchivePublicView: React.FC<ArchivePublicViewProps> = ({
  ...
  const [mediaSort, setMediaSort] = useState<'curated' | 'newest' | 'oldest' | 'highlights' | 'random'>('curated');
  ...
  const sortedAndFilteredMedia = useMemo(() => {
    ...
    // Creator-selected order is the audience default. Visitors can temporarily
    // choose another sort without changing the saved arrangement.
    if (mediaSort === 'curated') {
      list.sort((a, b) => (a.position ?? Number.MAX_SAFE_INTEGER) - (b.position ?? Number.MAX_SAFE_INTEGER)
        || new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    } else if (mediaSort === 'newest') {
      list.sort((a, b) => {
